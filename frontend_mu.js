...
class CacheManager {
  ...
  set(anio, mes, data) {
    this.cache.set(this.getKey(anio, mes), data);
  }
...
//gestion de peticiones a la api con control de concurrencia
class APIManager {
...
const main__ = async () => {
  ...
  //carga de datos con cache y deduplicacion
  const getDetencionesPorMesYAnio = async (anio, mes) => {
    ...
    //verificar si hay una peticion pendiente
  ...
  //carga paralela de multiples meses
  const cargarMultiplesMeses = async (anio, meses) => {
  const resultados = [];

  for (const mes of meses) {
    if (cacheManager.has(anio, mes)) {
      resultados.push(...cacheManager.get(anio, mes));
    } else {
      const datos = await getDetencionesPorMesYAnio(anio, mes);
      resultados.push(...datos);
      // Pequeña pausa no bloqueante para evitar sobrecarga de red
  ...
  //precarga de datos del año actual
  const precargarDatosActuales = async () => {
    const anioActual = new Date().getFullYear().toString();
    const mesesActual = CONFIG.MESES_DISPONIBLES[anioActual] || [];
    
    if (mesesActual.length > 0) {
      //precargar en segundo plano sin bloquear la ui
      setTimeout(() => {
        cargarMultiplesMeses(anioActual, mesesActual).catch(error => 
          console.log('Precarga en background:', error)
        );
      }, 100);
    }
  };
  ...
  const renderGraficoParetoInicial = async () => {
    ...
    //identificar meses unicos a cargar
  ...
  const renderTablaDetalle = (datos) => {
    ...
    tabla.innerHTML = `
      <h5 class="mb-3">Detalle de Detenciones (${datos.length} registros)</h5>
  ...
  precargarDatosActuales();
  
  //carga vista inicial de grafico pareto de los ultimos 7 dias
