...
confirmarEnvioEditar.addEventListener("click", async () => {
  ...
  const fechaInput = formNcEditar.fecha_deteccion.value;
  const fecha = fechaInput.split("-");
  const fechaFormateada = `${fecha[2]}-${fecha[1]}-${fecha[0]}`;

  const ncEditada = {
    USUARIO: user,
    N_FOLIO: formNcEditar.folio.value.trim(),
    MES: formNcEditar.mes.value.trim(),
    ANNIO: formNcEditar.anio.value.trim(),
    FECHA_DETECCION: fechaFormateada,
    AREA: formNcEditar.area.value.trim(),
    TIPO_NC: formNcEditar.tipo_nc.value.trim(),
    OBSERVACION: formNcEditar.observacion.value.trim(),
    ESTADO: formNcEditar.estado.value.trim(),
